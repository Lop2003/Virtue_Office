...
interface MultiplayerContextType {
  ...
  joinRoom: (name: string, outfit: AvatarOutfit, position: [number, number, number]) => void;
  sendMove: (position: [number, number, number], rotationY: number, isWalking: boolean) => void;
...
export const MultiplayerProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [claimedDesks, setClaimedDesks] = useState<Map<number, string>>(new Map());
  ...
  useEffect(() => {
    ...
    socket.on('player:moved', (data: {
      ...
      setRemotePlayers((prev) =>
        prev.map((p) =>
          p.id === data.id
            ? { ...p, position: data.position, rotationY: data.rotationY, isWalking: data.isWalking }
            : p
        )
      );
    });
    ...
    socket.on('player:desk', (data: { id: string; deskId: number | null }) => {
      setRemotePlayers((prev) =>
        prev.map((p) => (p.id === data.id ? { ...p, activeDesk: data.deskId } : p))
      );
    ...
    socket.on('player:volume', (data: { id: string; volume: number }) => {
      setRemotePlayers((prev) =>
        prev.map((p) => (p.id === data.id ? { ...p, volume: data.volume } : p))
      );
    });
    ...
    socket.on('player:chat', (data: {
      ...
      setRemotePlayers((prev) =>
        prev.map((p) => (p.id === data.id ? { ...p, chatMessage: data.text } : p))
      );
      // Auto-clear bubble after 5 s
      setTimeout(() => {
        setRemotePlayers((prev) =>
          prev.map((p) =>
            p.id === data.id && p.chatMessage === data.text ? { ...p, chatMessage: null } : p
          )
        );
      }, 5000);
    });
  ...
  const joinRoom = useCallback(
    (name: string, outfit: AvatarOutfit, position: [number, number, number]) => {
  ...
  const sendMove = useCallback(
    (position: [number, number, number], rotationY: number, isWalking: boolean) => {
