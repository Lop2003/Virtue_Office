...
const RP_TORSO_GEO  = new THREE.CylinderGeometry(0.24, 0.20, 0.55, 6);
const RP_NECK_GEO   = new THREE.CylinderGeometry(0.07, 0.07, 0.12, 6);
const RP_ARM_GEO    = new THREE.CylinderGeometry(0.06, 0.05, 0.40, 6);
const RP_HAND_GEO   = new THREE.SphereGeometry(0.055, 6, 6);
const RP_LEG_GEO    = new THREE.CylinderGeometry(0.07, 0.06, 0.50, 6);
const RP_FOOT_GEO   = new THREE.BoxGeometry(0.10, 0.08, 0.18);
...
const RP_GLASS_BRDG = new THREE.BoxGeometry(0.10, 0.02, 0.01);
...
const matCache: Record<string, THREE.MeshLambertMaterial | THREE.MeshStandardMaterial> = {};
const getLambertMat = (color: string) => {
  if (!matCache[`l_${color}`]) matCache[`l_${color}`] = new THREE.MeshLambertMaterial({ color });
...
const getStdMat = (color: string, roughness = 0.7) => {
  ...
  if (!matCache[key]) matCache[key] = new THREE.MeshStandardMaterial({ color, roughness });
...
export const OtherPlayer: React.FC<OtherPlayerProps> = ({ player }) => {
  ...
  const interpPos  = useRef(new THREE.Vector3(...player.position));
  const interpRotY = useRef(player.rotationY);

  useFrame((_state, delta) => {
    // Smooth position lerp toward server-authoritative position
    const target = new THREE.Vector3(...player.position);
    interpPos.current.lerp(target, Math.min(1, delta * 12));
    ...
    if (player.isWalking) {
      const speed = 6;
      const swing = 0.5;
      if (leftArmRef.current)  leftArmRef.current.rotation.x  =  Math.sin(t * speed) * swing;
      if (rightArmRef.current) rightArmRef.current.rotation.x = -Math.sin(t * speed) * swing;
      if (leftLegRef.current)  leftLegRef.current.rotation.x  = -Math.sin(t * speed) * swing * 0.8;
      if (rightLegRef.current) rightLegRef.current.rotation.x =  Math.sin(t * speed) * swing * 0.8;
    } else if (player.activeDesk !== null) {
      // Typing animation while seated
      const ts = 5;
      if (leftArmRef.current)  leftArmRef.current.rotation.x  = THREE.MathUtils.lerp(leftArmRef.current.rotation.x,  -Math.PI / 3 + Math.sin(t * ts) * 0.06, 0.15);
      if (rightArmRef.current) rightArmRef.current.rotation.x = THREE.MathUtils.lerp(rightArmRef.current.rotation.x, -Math.PI / 3 + Math.cos(t * ts) * 0.06, 0.15);
    ...
    <group ref={groupRef}>
      ...
      {player.chatMessage && (
        <Html position={[0, 1.85, 0]} center distanceFactor={8}>
          <div style={{ pointerEvents: 'none', userSelect: 'none' }} className="flex flex-col items-center">
      ...
      {/* ── Torso ──────────────────────────────────────────────────────────── */}
      <mesh position={[0, 0.70, 0]} castShadow geometry={RP_TORSO_GEO} material={getStdMat(clothingColor)} />

      {/* ── Neck ───────────────────────────────────────────────────────────── */}
      <mesh position={[0, 1.02, 0]} geometry={RP_NECK_GEO} material={getLambertMat(skinTone)} />

      {/* ── Left Arm ───────────────────────────────────────────────────────── */}
      <group ref={leftArmRef} position={[-0.29, 0.88, 0.02]} rotation={[0, 0, -0.1]}>
        <mesh geometry={RP_ARM_GEO} material={getLambertMat(clothingColor)} />
        <mesh position={[0, -0.22, 0]} geometry={RP_HAND_GEO} material={getLambertMat(skinTone)} />
      </group>

      {/* ── Right Arm ──────────────────────────────────────────────────────── */}
      <group ref={rightArmRef} position={[0.29, 0.88, 0.02]} rotation={[0, 0, 0.1]}>
        <mesh geometry={RP_ARM_GEO} material={getLambertMat(clothingColor)} />
        <mesh position={[0, -0.22, 0]} geometry={RP_HAND_GEO} material={getLambertMat(skinTone)} />
      </group>
      ...
      <group ref={leftLegRef} position={[-0.13, 0.27, 0]}>
        ...
        <mesh position={[0, -0.27, 0.04]} geometry={RP_FOOT_GEO} material={MAT_SHOE} />
      </group>
      ...
      <group ref={headRef} position={[0, 1.32, 0]}>
        <mesh castShadow geometry={RP_HEAD_GEO} material={getStdMat(skinTone, 0.5)} />

        {/* Eyes */}
        <mesh position={[-0.14, 0.04, 0.255]} geometry={RP_EYE_GEO} material={MAT_EYE} />
        <mesh position={[ 0.14, 0.04, 0.255]} geometry={RP_EYE_GEO} material={MAT_EYE} />

        {/* Hair */}
        <mesh position={[0, 0.20, 0.05]} geometry={RP_HAIR_GEO} material={getLambertMat(hairColor)} />

        {/* Glasses */}
        {hasGlasses && (
          <group position={[0, 0.04, 0.26]}>
            <mesh position={[-0.14, 0, 0]} geometry={RP_GLASS_RIM} material={MAT_GLASS} />
            <mesh position={[ 0.14, 0, 0]} geometry={RP_GLASS_RIM} material={MAT_GLASS} />
            <mesh position={[0, 0, 0]}     geometry={RP_GLASS_BRDG} material={MAT_GLASS} />
          </group>
        )}
        ...
        {hasHeadphones && (
          <group position={[0, 0.04, 0]}>
            ...
            <mesh position={[-0.26, 0, 0]} rotation={[0, 0,  Math.PI / 2]} geometry={RP_HP_CUP} material={MAT_HP_CUP_M} />
            <mesh position={[ 0.26, 0, 0]} rotation={[0, 0, -Math.PI / 2]} geometry={RP_HP_CUP} material={MAT_HP_CUP_M} />
          </group>
        )}
      </group>
