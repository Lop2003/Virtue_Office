  ...
  useFrame((state) => {
    ...
    if (isWalking) {
      ...
      groupRef.current.position.y =
        yOffset + Math.abs(Math.sin(t * 9.5)) * 0.08;
...
export const Avatar: React.FC<AvatarProps> = ({
  ...
  const [activeThoughtEmoji, setActiveThoughtEmoji] = useState<string | null>(
    null,
  );
        ...
        if (activeDesk !== null) {
          ...
          for (const desk of desks) {
            if (desk.hasColleague === false) {
              ...
              const distSq =
                (currentPos.current.x - cx) * (currentPos.current.x - cx) +
                (currentPos.current.z - cz) * (currentPos.current.z - cz);
              if (distSq < 1.2 * 1.2) {
                foundDeskId = desk.id;
                break;
              }
  ...
  useEffect(() => {
    ...
    const reactionAction =
      actions["Yes"] || actions["Wave"] || actions["ThumbsUp"];
  ...
  useEffect(() => {
    ...
    const waveInterval = setInterval(() => {
      ...
      if (humanLeftArmRef.current && humanRightArmRef.current) {
        humanLeftArmRef.current.rotation.x =
          -Math.PI + Math.sin(timer * 15.0) * 0.5;
        humanRightArmRef.current.rotation.x =
          -Math.PI - Math.sin(timer * 15.0) * 0.5;
      }
    }, 50);
  ...
  useFrame((state, delta) => {
    ...
    if (hasKeyboardInput) {
      ...
      const walkSpeed = 3.2;
      const nextPos = currentPos.current
        .clone()
        .addScaledVector(moveDir, delta * walkSpeed);
      currentPos.current.copy(
        resolveMovement(currentPos.current, nextPos, activeDesk),
      );
      ...
      if (wp) {
        ...
        if (step >= dist) {
          currentPos.current.copy(
            resolveMovement(currentPos.current, wp, activeDesk),
          );
    ...
    // Periodic check + multiplayer position broadcast (~10×/s)
    if (time - lastStateCheck.current > 0.1) {
      lastStateCheck.current = time;

      if (onMove) {
        const isCurrentlyWalking =
          hasKeyboardInput || (isWalking && pathRef.current.length > 0);
        onMove(
    ...
    if (
      ...
      !isWalking &&
      volume > 0.35 &&
      time - lastSpawnTime.current > 1.5
    ) {
    ...
    if (outfit.type === "human") {
      if (isWalking) {
        ...
        if (humanLeftLegRef.current) {
          humanLeftLegRef.current.rotation.x =
            Math.sin(time * swingSpeed) * swingAngle;
        }
        if (humanRightLegRef.current) {
          humanRightLegRef.current.rotation.x =
            -Math.sin(time * swingSpeed) * swingAngle;
        }
        if (humanLeftArmRef.current) {
          humanLeftArmRef.current.rotation.x =
            -Math.PI / 6 - Math.sin(time * swingSpeed) * (swingAngle * 0.7);
        }
        ...
        if (isSeated) {
          ...
          if (humanLeftArmRef.current && humanRightArmRef.current) {
            humanLeftArmRef.current.rotation.x =
              -Math.PI / 3.2 + Math.sin(time * typeSpeed) * typingIntensity;
            humanLeftArmRef.current.rotation.z =
              -0.15 + Math.cos(time * typeSpeed) * 0.02;
            humanRightArmRef.current.rotation.x =
              -Math.PI / 3.2 +
              Math.cos(time * typeSpeed * 1.1) * typingIntensity;
            humanRightArmRef.current.rotation.z =
              0.15 + Math.sin(time * typeSpeed) * 0.02;
          }
        } else {
        ...
        if (humanHeadRef.current) {
          ...
          humanHeadRef.current.rotation.x =
            Math.sin(time * 1.5) * 0.03 + speakNod;
  ...
  const positionOffsetY = isSeated
    ? outfit.type === "robot"
      ? 0.45
      : 0.38
    : 0;
  ...
  const modelForwardFix = ["xbot", "cesium-man"].includes(outfit.characterId)
    ? 0
    : Math.PI;
      ...
      <group position={[0, positionOffsetY, positionOffsetZ]}>
        ...
        {nearbyDeskId !== null && activeDesk === null && (
          <Html position={[0, outfit.type === "robot" ? 1.4 : 1.7, 0]} center>
            <div
              className={`backdrop-blur-sm px-2.5 py-1 rounded-full text-[11px] font-semibold shadow-lg flex items-center gap-1.5 animate-bounce select-none pointer-events-none border transition-all duration-500 ${
                theme === "night"
            ...
            >
              <span
                className={`px-1.5 py-0.5 rounded text-[9px] uppercase font-mono border shadow-inner transition-all duration-500 ${
                  theme === "night"
